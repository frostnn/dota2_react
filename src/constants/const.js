//API pandascore
export const API_URL = 'https://api.pandascore.co';
export const API_TOKEN = 'token=KoYvfhoqjN6mLLGXHywFceVAZUVSa5GrZkjK808OX2RAEKj9OMU';
export const API_HEROES_PAGE_ONE = '/dota2/heroes?per_page=100&';
export const API_HEROES_PAGE_TWO = '/dota2/heroes?per_page=100&page=2&';
export const API_LEAGUES = '/dota2/leagues?per_page=100&';
export const API_ITEMS = '/dota2/items?per_page=100&';
export const API_MATCHES = '/dota2/matches?per_page=100&';
export const API_PLAYERS = '/dota2/players?per_page=100&';
export const API_LIVES = '/lives?';

//API opendota
export const API_HEROES_STAT = 'https://api.opendota.com/api/heroStats';
export const API_TEAMS = 'https://api.opendota.com/api/teams';
export const API_PRO_PLAYERS = 'https://api.opendota.com/api/proPlayers';
